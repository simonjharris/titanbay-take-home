...
async def get_investments_for_fund(fund_id: UUID, db: DatabaseSession) -> list[InvestmentRead]:
    try:
...
async def create_investment_for_fund(fund_id: UUID, investment_data: InvestmentCreate, db: DatabaseSession) -> InvestmentRead:
